...
def eval_unseen_classes(netR, data, opt, batch_size=8):
    ...
    for i in range(0, data.ntest_unseen, batch_size):
        ...
        if opt.cuda:
            ...
            # netE1
        # with torch.no_grad():
        #     img_embed, _, _, _, _ = netE(test_feature, att_empty_E1, att_empty_E2)

        # generate words
        #img_binary = torch.sigmoid(img_binary)
        outputs = netR.generate_sentence(test_feature, start_word, end_word, states=(lstm1_states, lstm2_states, lstm3_states),
...
optimizerR = optim.Adam(netR.parameters(), lr=0.000005, betas=(opt.beta1, 0.999))
##
best_acc_zsl = 0
...
for epoch in range(opt.nepoch):
    ...
    for i in range(opt.n_batch):
        ...
        with torch.no_grad():
            img_embed, _, _, img_binary, _ = netE(input_res, input_att, input_att_confuse)
        # L2 norm
        # img_embed_com_l2 = func.normalize(img_embed_com, p=2, dim=1)
        # img_embed_spe_l2 = func.normalize(img_embed_spe, p=2, dim=1)
        # text_embed_l2 = func.normalize(text_embed, p=2, dim=1)
        # att_concat_embed_l2 = func.normalize(att_concat_embed, p=2, dim=1)
        # ## compute sim loss
        # E_sim_cost = 0.5 * compute_sim_loss(img_embed_com_l2, att_concat_embed_l2) + 1.0 * compute_sim_loss(img_embed_com_l2, text_embed_l2)
...
# torch.save(netE.state_dict(), os.path.join(opt.save_path, opt.save_name + '_TwoWay_Embed_Explain:netE_Epoch-{}.ckpt'.format(epoch)))
torch.save(netR.state_dict(),os.path.join(opt.save_path, opt.save_name + '_OneWay_Explain_New_only_image_binary:netE_Epoch-199:netR_Epoch-{}.ckpt'.format(epoch)))
